...
impl<I2C: I2c> Tsc2007<I2C> {
    ...
    fn command(&mut self, function: u8, power: u8, resolution: u8) -> Result<u16, I2C::Error> {
        self.cmd[0] = (function & 0x0F) << 4;
        self.cmd[0] |= (power & 0x03) << 2;
        self.cmd[0] |= (resolution & 0x01) << 1;

        return match self.i2c.write_read(TSC2007_ADDR, &self.cmd, &mut self.buf) {
            Ok(()) => {
    ...
    pub fn touched(&mut self) -> bool {
        if let Ok(point) = self.touch() {
    ...
    pub fn touch(&mut self) -> Result<(u16, u16, u16), I2C::Error> {
        let x = self.command(TSC2007_MEASURE_X, TSC2007_ADON_IRQOFF, TSC2007_ADC_12BIT)?;
        let y = self.command(TSC2007_MEASURE_Y, TSC2007_ADON_IRQOFF, TSC2007_ADC_12BIT)?;
        let z = self.command(TSC2007_MEASURE_Z1, TSC2007_ADON_IRQOFF, TSC2007_ADC_12BIT)?;
