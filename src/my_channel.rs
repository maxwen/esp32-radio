...
pub struct MyChannel<M, T, const N: usize>
    where
        M: RawMutex,
{
...
impl<M, T, const N: usize> MyChannel<M, T, N>
    where
        M: RawMutex,
{
    ...
    pub fn free(&self) -> usize {
        self.lock(|c| c.queue.capacity() - c.queue.len())
    }
}
...
impl<M, T, const N: usize> DynamicChannel<T> for MyChannel<M, T, N>
    where
        M: RawMutex,
{
    fn try_send_with_context(&self, m: T, cx: Option<&mut Context<'_>>) -> Result<(), TrySendError<T>> {
        MyChannel::try_send_with_context(self, m, cx)
    }

    fn try_receive_with_context(&self, cx: Option<&mut Context<'_>>) -> Result<T, TryReceiveError> {
        MyChannel::try_receive_with_context(self, cx)
    }

    fn poll_ready_to_send(&self, cx: &mut Context<'_>) -> Poll<()> {
        MyChannel::poll_ready_to_send(self, cx)
    }

    fn poll_ready_to_receive(&self, cx: &mut Context<'_>) -> Poll<()> {
        MyChannel::poll_ready_to_receive(self, cx)
    }

    fn poll_receive(&self, cx: &mut Context<'_>) -> Poll<T> {
        MyChannel::poll_receive(self, cx)
    }
}
...
mod tests {
    ...
    fn simple_send_and_receive() {
        let c = MyChannel::<NoopRawMutex, u32, 3>::new();
        assert!(c.try_send(1).is_ok());
        assert_eq!(c.try_receive().unwrap(), 1);
    }
